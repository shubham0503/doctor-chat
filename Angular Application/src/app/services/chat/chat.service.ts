...
export class ChatService {
	...
	sendRequests(params: DoctorRequest): Observable<Request> {
		return this.http.post(`${this.BASE_URL}sendRequest`, JSON.stringify(params), this.httpOptions).pipe(
			map(
				(response: Request) => {
					return response;
				},
			...
			if (userId !== null && userId !== undefined) {
				...
				.subscribe(
					(response: UserSessionCheck) => {
						...
						localStorage.setItem('usergroup', response.usergroup);
						observer.next(true);
					}, (error) => {
						this.router.navigate(['/']);
						observer.next(false);
