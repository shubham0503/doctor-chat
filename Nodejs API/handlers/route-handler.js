...
class RouteHandler {
    ...
    async registerRouteHandler(request, response) {
        const data = {
            username: (request.body.username).toLowerCase(),
            password: request.body.password,
            usergroup: request.body.usergroup
        };
        if (data.username === '') {
            response.status(CONSTANTS.SERVER_ERROR_HTTP_CODE).json({
                error: true,
            ...
            response.status(CONSTANTS.SERVER_ERROR_HTTP_CODE).json({
                error: true,
                message: CONSTANTS.USERGROUP_NOT_FOUND
            });
        } else {
            try {
                data.online = 'Y';
    ...
    async sendRequestHandler(request, response) {
        const data = {
        ...
        if (data.userid === '') {
            ...
            try {
                data.acceptedBy = '';
                const result = await queryHandler.sendRequest(data);
                if (result === null || result === undefined) {
                    response.status(CONSTANTS.SERVER_OK_HTTP_CODE).json({
                        error: false,
                        message: CONSTANTS.SEND_REQUEST_FAILED
                    });
                } else {
                    response.status(CONSTANTS.SERVER_OK_HTTP_CODE).json({
                        error: false,
                        requestId: result.request_id,
                        message: CONSTANTS.SEND_REQUEST_OK
                    });
                }
            } catch (error) {
                response.status(CONSTANTS.SERVER_NOT_FOUND_HTTP_CODE).json({
